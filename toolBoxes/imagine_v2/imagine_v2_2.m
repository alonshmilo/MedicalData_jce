function hImgAxes = imagine_v2_2(varargin)
...
% =========================================================================
imfuse_alpha1 = 2;
imfuse_alpha2_start = 0.2;
imfuse_alpha2 = imfuse_alpha2_start;
...
%   
                if(~isempty(SData(iSeriesInd).iSegmention))
                    ...
                    imfuse_alpha1
                    imfuse_alpha2
                    dThisImg = imfuse(dThisImg*imfuse_alpha1, rgbiMask*imfuse_alpha2,'blend','Scaling','joint' );
            ...
            if iSeriesInd <= length(SData)
                ...
                if(~isempty(SData(iSeriesInd).iSegmention))
                   ...
                   % get(SImg.hSegImg(iAxisInd))    
                    iImg = imfuse(iImg*imfuse_alpha1, rgbiMask*imfuse_alpha2,'blend','Scaling','joint' );
                end
...
% % % % %         
    ...
    function fWindow(iD)
        for i = 1:length(SData)
            ...
            %imfuse_alpha1 = imfuse_alpha1 - 0.1;
           ...
           %  imfuse_alpha1 =  dRealCenter/1000000
            imfuse_alpha2 =  imfuse_alpha2_start - dRealWidth/100000
   
%   
                if(~isempty(SData(i).iSegmention)) 
                    % SData(i).iActiveImage
                    dThisImg = SData(i).iImg(:,:,SData(i).iActiveImage);
                    iMask = SData(i).iSegmention(:,:,SData(i).iActiveImage);
                    rgbiMask =  label2rgb(iMask);   
                    dThisImg = imfuse(dThisImg*imfuse_alpha1, rgbiMask*imfuse_alpha2,'blend','Scaling','joint' );
                    set(SImg.hImg(i), 'CData', dThisImg); 
